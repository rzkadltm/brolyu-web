...
**Auth** — `AuthProvider` (`src/contexts/AuthProvider.tsx`) hydrates the current user from a bearer token in `localStorage` (`brolyu_token`) by calling `GET /auth/me`. Sign-in is **Google OAuth only** — the client redirects to `${VITE_API_BASE_URL}/auth/oauth/google`, the backend handles the exchange, and `AuthCallback` applies the returned token. Email/password code paths exist but are commented out. Use the `useAuth()` hook from `src/contexts/useAuth.ts` to read user state and call sign-in/profile/avatar mutations.

**API client** — `src/lib/api.ts` exposes a single `api` object plus `tokenStore`, `ApiError`, and shared types (`User`, `AuthSuccess`). It auto-attaches `Authorization: Bearer <token>` and base-URLs against `VITE_API_BASE_URL`. Add new endpoints here, not via ad-hoc `fetch` calls.
